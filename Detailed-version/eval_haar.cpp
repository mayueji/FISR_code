...
void eval_clear(int start, int end, OctNode n, int size, int z, float val)
{
		// compute values
	for (int child = 0; child < 8; child++)
	{
		n.func_vals_linear(child) = 0;
	}
	...
	if (high)
	{
		...
		{
			if (n.children(child&1,child>>1,1).data)
				eval_haar(start, end, n.children(child&1,child>>1,1), size2, z+size2, n.func_vals(child&1,child>>1,0));
		}
	}
...
void eval_clear(int start, int end)
{
	...
	int z = 0;
	OctNode n = g.tree.root;
	eval_clear(start, end, n, size, z, 0);
}