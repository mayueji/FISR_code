...
float OctNode::getValueAtPointhaar(vect3f &pos, vect3f mine, vect3f ext,float val)
{
	vect3f ext2 = ext*.5;
	vect3f center = mine + ext2;
	...
	int child=ix+(iy<<1)+(iz<<2);	
		
	func_vals_linear(child) = val;

	for (int coeff = 0; coeff < 8; coeff++)
	{
		int _and = coeff & child;
		func_vals_linear(child) += ((_and >> 2) ^ (_and >> 1) ^ _and) & 1 ? - coeffs_linear(coeff) : coeffs_linear(coeff);
	}	
	value=func_vals_linear(child);
	
	if (children(ix, iy, iz).data)
	{
		vect3f corner2(mine[0] + ext2[0]*ix, mine[1] + ext2[1]*iy, mine[2] + ext2[2]*iz);
		value=children(ix, iy, iz).getValueAtPointhaar(pos, corner2, ext2,func_vals_linear(child));
	}
