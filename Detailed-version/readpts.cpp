...
void readds(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, int which_stream)
{
	if (read_from_buffer[which_stream])
	{
		pt_read = pt_buffer[which_stream];
...
void readPoint2(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, int which_stream,int flag)
{
	if (read_from_buffer[which_stream])
	{
		pt_read = pt_buffer[which_stream];
...
void readPointtrue(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, int which_stream)
{
	if (read_from_buffer[which_stream])
	{
		pt_read = pt_buffer[which_stream];
...
void readPointnonormal(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, int which_stream)
{
	if (read_from_buffer[which_stream])
	{
		pt_read = pt_buffer[which_stream];
...
bool readdsUpTo(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, OctTree &tree, int &pts_index, int pts_num, int which_stream, int upto)
{
	if (pts_index < pts_num)
	{
		readds(pt_read, file, center, scale_factor, which_stream);
	}
	else
		return false; // we are past the end of the file
}
...
bool readPointUpTotrue(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, OctTree &tree, int &pts_index, int pts_num, int which_stream, int upto)
{
	if (pts_index < pts_num)
	{
		readPointtrue(pt_read, file, center, scale_factor, which_stream);
...
bool readPointUpTononormal(OrientedPoint &pt_read, ifstream &file, const vect3f& center, const float scale_factor, OctTree &tree, int &pts_index, int pts_num, int which_stream, int upto)
{
	if (pts_index < pts_num)
	{
		readPointnonormal(pt_read, file, center, scale_factor, which_stream);
